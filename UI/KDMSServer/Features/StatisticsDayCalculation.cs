...
{
    public class StatisticsDayCalculation
    {
        ...
        public class CommandHandler : IRequestHandler<Command, Response>
        {
            ...
            public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    var datas = _commonData.StatisticsHourDataLoad(request.Time);
                    if (datas != null && datas.Count > 0)
                    {
                        List<StatisticsDay> dataList = new List<StatisticsDay>();
                        ...
                        foreach (var ceq in ceqList)
                        {
                            ...
                            var inputData = findDatas.Select(p => new StatisticsDay
                            {
                                SaveTime = request.Time.AddDays(1),
                                Ceqid = p.Ceqid,
                                Cpsid = p.Cpsid,
                        ...
                        if (dataList.Count > 0)
                            _commonData.StatisticsDayDataInput(dataList);

...
{
    public class StatisticsMonthCalculation
    {
        ...
        public class CommandHandler : IRequestHandler<Command, Response>
        {
            ...
            public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    var datas = _commonData.StatisticsDayDataLoad(request.Time);
                    if (datas != null && datas.Count > 0)
                    {
                        List<StatisticsMonth> dataList = new List<StatisticsMonth>();
                        ...
                        foreach (var ceq in ceqList)
                        {
                            ...
                            var inputData = findDatas.Select(p => new StatisticsMonth
                            {
                                SaveTime = request.Time.AddMonths(1),
                                Ceqid = p.Ceqid,
                                Cpsid = p.Cpsid,
                        ...
                        if (dataList.Count > 0)
                            _commonData.StatisticsMonthDataInput(dataList);
